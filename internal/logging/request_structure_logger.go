...
import (
	"bitbucket.org/libertywireless/circles-framework/clmiddlewares"
	"fmt"
...
func (l *RequestStructureLogger) NewStructuredEntry(r *http.Request) CustomLogEntry {
	entry := &StructuredLoggerEntry{logger: logrus.NewEntry(l.logger)}
	...
	if reqID := clmiddlewares.GetReqID(r.Context()); reqID != "" {
		logFields["req_id"] = reqID
	}
