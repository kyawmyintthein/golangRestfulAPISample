...
func New(cfg *NewrelicCfg, opts ...option.Option) (NewrelicTracer, error) {
	...
	if lgr == nil && !ok {
		newrelicTracer.logger = logging.DefaultKVLogger()
	} else {
	...
	if err != nil {
		...
		newrelicTracer.logger.Error(err, "[CLNewrelic] failed to initiate new-relic tracer")
		return newrelicTracer, err
	}
	newrelicTracer.app = app
	newrelicTracer.cfg.Enabled = true
	newrelicTracer.logger.Info("[CLNewrelic] Newrelic Tracer is successfully initiated")
...
func (service *newrelicTracer) ChiMiddleware(next http.Handler) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok {
			service.logger.Infof("URL: '%s' is skip")
		}
...
func (service *newrelicTracer) RecordExternalMetric(r *http.Request, opName string) (*newrelic.ExternalSegment, error) {
	if !service.cfg.Enabled || service.app == nil {
		return nil, fmt.Errorf("[CLNewrelic] new-relic tracer is not avaliable")
	}
