...
func NewMongodbConnector(cfg *MongodbConfig, opts ...cloption.Option) (MongodbConnector, error) {
	...
	if lgr == nil && !ok {
		mongodbConnector.logger = cllogging.DefaultKVLogger()
	} else {
		mongodbConnector.logger = lgr
	}
...
func (this *mongodbConnector) connect() error {
	...
	connectOnce.Do(func() {
		...
		if err != nil {
			this.logger.Errorf(err, "Failed to connect to database: %s", this.cfg.DatabaseName)
			return
		}
...
func (this *mongodbConnector) DB(ctx context.Context) *mongo.Database {
	...
	if err != nil{
		this.logger.Errorf(err, "fail to ping %s", this.cfg.DatabaseHosts)
	}
	return this.db
}
